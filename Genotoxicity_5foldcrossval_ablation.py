...
class MyCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        return
    def savedata(self,foldnum,lr,batchsize,wd,dd,datatype):
        ...
        df.to_csv('GenotoxNet_data/ablation_model/%s/loss/lr%s_batch%s_dropout%s_%sl2_loss_%s.csv'%(datatype,lr,batchsize,dd,wd,foldnum),index=False,header=True)
        return
...
def ModelTraining(model,lr,batchsize,wd,dd,X_drug_data_train,X_gexpr_data_train,X_assay_data_train,Y_train,validation_data,foldnum,datatype,nb_epoch=100):
    ...
    callbacks = [ModelCheckpoint('GenotoxNet_data/ablation_model/weights_%s_{epoch:04d}.h5'%foldnum, monitor='val_average_precision', verbose=1, save_weights_only=True,period=1),history] #,history
    model.fit(x=[X_drug_feat_data_train,X_drug_adj_data_train,X_gexpr_data_train,X_assay_data_train],y=Y_train,batch_size=batchsize,epochs=nb_epoch,validation_data=(validation_data[0],validation_data[1]),callbacks=callbacks)#validation_split=0
    history.savedata(foldnum,lr,batchsize,wd,dd,datatype)
    return model
...
def main():
    for datatype in ['structure+transcript','transcript+assay','structure','structure+assay','transcript','assay']:
            ...
            for batchsize in [16,32,64]:#
                for dd in [0,0.1,0.2]:#
                    for wd in [0.0001,0.001,0]:#
                        ...
                        df1.to_csv('GenotoxNet_data/ablation_model/%s/lr%s_batch%s_dropout%s_%sl2.csv'%(datatype,lr,batchsize,dd,wd),index=False,header=True)
