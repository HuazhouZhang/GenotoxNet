...
from Genotoxicity_model_differenttype import KerasMultiSourceGCNModel
...
class MyCallback(Callback):
    def __init__(self,validation_data,extra_data,patience):
        self.x_val = validation_data[0]
        self.y_val = validation_data[1]
        self.x_extra = extra_data[0]
        self.y_extra = extra_data[1]
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        y_pred_extra = self.model.predict(self.x_extra)
        roc_extra = roc_auc_score(self.y_extra, y_pred_extra)
        precision,recall,_, = metrics.precision_recall_curve(self.y_val,y_pred_val)
        pr_val = -np.trapz(precision,recall)
        val_loss1 = logs.get('val_loss')
        self.val_loss['epoch'].append(logs.get('val_loss'))
        self.aucl['epoch'].append(roc_val)
        self.aucextra['epoch'].append(roc_extra)
        ...
        return
    def savedata(self,lr,batchsize,wd,dd,datatype):
        ...
        for ii in iters:
            ...
            aucrocextra = self.aucextra['epoch'][ii]
            dflist.append([ii+1,aucroc,yloss,testloss,aucrocextra])
        df = pd.DataFrame(dflist)
        df.columns = ['epoch','auc','train_loss','validation_loss','testextra']
        df.to_csv('GenotoxNet_data/bestmodel/lr%s_batch%s_dropout%s_%sl2_loss_%s.csv'%(lr,batchsize,dd,wd,datatype),index=False,header=True)
        return

def ModelTraining(model,lr,batchsize,wd,dd,datatype,X_drug_data_train,X_gexpr_data_train,X_assay_data_train,Y_train,validation_data,extra_data,nb_epoch=300):
    ...
    history = MyCallback(validation_data=validation_data,extra_data=extra_data,patience=60)#60
    ...
    history.savedata(lr,batchsize,wd,dd,datatype)
    return model
...
def main():
    dflist = []
    auclist = []
    list1 = [['all',0.0001,16,0.1,0.0001],['structure+transcript',0.0001,16,0.1,0.001],['structure',0.001,16,0.2,0],['structure+assay',0.0001,16,0,0],['assay',0.01,16,0.2,0],['transcript+assay',0.001,16,0.2,0.001],['transcript',0.001,16,0.1,0.001]]
    for parames in list1:
        sed = 225
        datatype,lr,batchsize,dd,wd = parames[0],parames[1],parames[2],parames[3],parames[4]
        print(datatype,lr,batchsize,dd,wd)
        random.seed(0)
        tf.set_random_seed(1)
        os.environ['TF_DETERMINISTIC_OPS'] = '1'
        np.random.seed(2)
        ...
        extra_data = [[X_drug_feat_data_extra,X_drug_adj_data_extra,X_gexpr_data_extra,X_assay_data_extra],Y_extra]
        ...
        args.unit_list,wd,dd,datatype,args.use_relu,args.use_bn,args.use_GMP)
        print('Begin training...')
        model = ModelTraining(model,lr,batchsize,wd,dd,datatype,X_drug_data_train,X_gexpr_data_train,X_assay_data_train,Y_train,validation_data,extra_data,nb_epoch=300)
        auROC_train,auPR_train,Y_pred_train = ModelEvaluate(model,X_drug_data_train,X_gexpr_data_train,X_assay_data_train,Y_train)
        auROC_val,auPR_val,Y_pred_val = ModelEvaluate(model,X_drug_data_validation,X_gexpr_data_validation,X_assay_data_validation,Y_validation)
        auROC_extra,auPR_extra,Y_pred_extra = ModelEvaluate(model,X_drug_data_extra,X_gexpr_data_extra,X_assay_data_extra,Y_extra)
        vallist = [Y_validation,Y_extra]
        dflist.append([datatype,lr,batchsize,dd,wd,auROC_val,auPR_val,auROC_extra,auPR_extra])
    df = pd.DataFrame(dflist)
    df.columns = ['type','lr','batchsize','Dropout','l2','roc_val','aupr_val','roc_extra','aupr_extra']
    df.to_csv('GenotoxNet_data/train_validation_test_result_ablation.csv',index=False)
